...
module.exports.deployContracts = async function deployContracts(
    ...
    for (let i in deployParam) {
      if (Array.isArray(deployParam[i])) {
        ...
        for (let k in deployParam[i]) {
          subParamArray = subParamArray.concat(
            getParamValue(deployParam[i][k]),
          );
        }
  ...
  for (let i = 0; i < contractsToDeploy.length; i++) {
    ...
    if (contract.type === "diamond") {
      ...
      console.log(
        "DigitalTokenTradeDiamond deployed:",
      ...
      for (const FacetName of facetNames) {
        ...
        if (
          ...
          FacetName == "DiamondEtherscanFacet"
        ) {
          cut.push({
      ...
      console.log("DigitalTokenTradeDiamond Cut:", cut);
      const diamondCut = await ethers.getContractAt(
        "IDiamondCut",
        await diamond.getAddress(),
      );
      let tx = await diamondCut.diamondCut(cut, ethers.ZeroAddress, "0x");
      console.log("DigitalTokenTradeDiamond cut tx: ", tx.hash);
      ...
      address[contract.contractName].contractDeployTxBlockNumber = (
        await ethers.provider.getTransaction(
          diamond.deploymentTransaction().hash,
        )
      ).blockNumber;
      ...
      contractInstance =
        contract.deployParam.length === 0
          ? await upgrades.deployProxy(factory, options)
          : await upgrades.deployProxy(factory, param, options);
      contractInstance = await contractInstance.waitForDeployment();
      console.log(
        contract.contractName + " proxy deployed to:",
...
module.exports.upgradeContracts = async function upgradeContracts(
  contractsToUpgrade,
) {
  const address = await db.read();
  const envParam = await db.readConfig();
  ...
  for (let i = 0; i < contractsToUpgrade.length; i++) {
    ...
    if (contract.type == "uups") {
      ...
      console.log(
        contract.contractName + " upgraded successfully",
        await contractInstance.getAddress(),
        "==>",
        contractImplAddress,
      );
      ...
      let selectors = [];
      const ignoredFacetNames = new Set([
        "DiamondLoupeFacet",
        "DiamondCutFacet",
      ]);
      for (const Facet of contract.facetNames) {
        if (ignoredFacetNames.has(Facet)) {
          continue;
        }
        for (const facetContract of address[contract.contractName]
          .facetContracts) {
      ...
      for (const FacetName of contract.facetNames) {
        ...
        for (
          let i = 0;
          i < address[contract.contractName].facetContracts.length;
          i++
        ) {
          if (
            address[contract.contractName].facetContracts[i].facetName ==
            FacetName
          ) {
            address[contract.contractName].facetContracts[i].address =
...
module.exports.verifyContracts = async function verifyContracts(
  contractsToVerify,
) {
  const address = await db.read();
  const envParam = await db.readConfig();
  ...
  for (let i = 0; i < contractsToVerify.length; i++) {
    ...
    if (contract.type == "diamond") {
      ...
      // 部署模拟合约
      const dumpFactory = await ethers.getContractFactory(
      ...
      // 绑定钻石合约和模拟合约的代理关系
      const etherscanFacet = await ethers.getContractAt(
      ...
      console.log(
        "etherscanFacet Implementation: ",
        await etherscanFacet.implementation(),
      );
      // 验证dump合约
      let cp = contract.contractPath.slice(
        0,
        contract.contractPath.lastIndexOf("/"),
      );
